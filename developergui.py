...
class Button():
	...
	def draw(self, color = (0, 0, 0)):
		...
		#surf.blit(self.surf, (self.xpos - self.surf.get_width() // 2, self.ypos + self.surf.get_height()))
...
class GUI():
	def __init__(self, height = 900, width = 900):
		pygame.init()
		self.height = height
		self.width = width
...
'''def main():
	board = GUI()
	exit = Button("Exit",400, 400, board.screen)
	textbox = TextBox(200, 200, board.screen)
	text = ''

	colorActive = (255, 255, 255)
	colorPassive = (255, 255, 0)
	color = colorPassive
	textBoxactive = False
	...
	while board.gameRunning:
		...
		for event in pygame.event.get():
			if event.type == pygame.QUIT:
				...
				if textbox.box.collidepoint(position):
					textBoxactive = True

				else:
					textBoxactive = False

				if exit.button.collidepoint(position):
					print(text)
		...
		textbox.textBox(text, color)

		#board.menu.mainloop(board.screen, bgfun = board.bgcolor)
		#pygame.display.flip()
		# Update screen
