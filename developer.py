...
class Nim():
	...
	def setMode1(self, difficultyMenu):
		global endGame
		endGame = False
		self.mode = 1
		self.saveNames(difficultyMenu)
		play(game)
		endGame = True
	...
	def menuSetup(self):
		...
		difficultyMenu = pygame_menu.Menu(
		...
		for i in range(101):
			difficultyList.append((str(i), i))

		difficultyMenu.add_selector("Difficulty : ", difficultyList, onchange = self.setDifficulty)

		difficultyMenu.add_text_input(
			"Player Name : ",
			default = "Human",
			textinput_id = "playerName"
			)

		difficultyMenu.add_button("Play", self.setMode1, difficultyMenu)

		difficultyMenu.add_button("Back", pygame_menu.events.BACK)

		difficultyMenu.add_button("Quit", pygame_menu.events.EXIT)
		...
		PvsPMenu.add_button("Play", self.setMode2, PvsPMenu)

		PvsPMenu.add_button("Back", pygame_menu.events.BACK)

		PvsPMenu.add_button("Quit", pygame_menu.events.EXIT)
		...
		modeMenu.add_button("Against AI", difficultyMenu)

		modeMenu.add_button("Against another Player", PvsPMenu)

		modeMenu.add_button("Quit", pygame_menu.events.EXIT)
		#modeMenu.disable()
		...
		againMenu.add_button("Yes", self.playAgain)

		againMenu.add_button("No", pygame_menu.events.EXIT)
		...
		while True:
			...
			if not endGame and modeMenu.is_enabled():
				...
				if(self.winner == self.player1):
					...
					if self.mode == 1:
						self.gameDisplay.write(self.gameDisplay.width // 2, 100, "AI won!")
					else:
	...
	def setDifficulty(self, title, difficulty):
	...
	def isBalanced(self, tempBoard):
		balanced = (tempBoard[0] ^ tempBoard[1]) ^ (tempBoard[2] ^ tempBoard[3]) ^ (tempBoard[4] ^ tempBoard[5])
	...
	def AImove(self):
		...
		if random.randint(0,101) > self.difficulty:
